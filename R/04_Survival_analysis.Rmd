...
N_nests <- NBI_breeding_all %>% count(breed_year)


NBI_breeding_year <- NBI_breeding_all %>% group_by(breed_year) %>% summarize_if(is.numeric, sum, na.rm = TRUE)
NBI_breeding_year <- NBI_breeding_year[,-c(2,3)]
NBI_breeding_year$n_nests <- N_nests$n
```

```{r}
NBI_breeding_SD <- NBI_breeding_all %>% group_by(breed_year) %>% mutate(SD_eggs = sd(b_count_eggs),SD_chicks = sd(b_count_hatching), SD_fledg = sd(b_count_fleding)) 
...
NBI_breeding_year$fledg_per_nest <- NBI_breeding_year$b_count_fleding/NBI_breeding_year$n_nests
...
NBI_breeding_year$surv_egg_fledg <- NBI_breeding_year$b_count_fleding/NBI_breeding_year$b_count_eggs

## survival chick to fledgling
NBI_breeding_year$surv_chick_fledg <- NBI_breeding_year$b_count_fleding/NBI_breeding_year$b_count_hatching
```
...
sum(NBI_breeding_year$b_count_fleding)
...
## Survival egg to fledgling
sum(NBI_breeding_year$b_count_fleding)/sum(NBI_breeding_year$b_count_eggs)
## Survival chick to fledgling
sum(NBI_breeding_year$b_count_fleding)/sum(NBI_breeding_year$b_count_hatching)
