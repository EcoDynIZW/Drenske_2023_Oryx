...
NBI_timesplit_sex$TimeStop <- NBI_timesplit_sex$tstop
NBI_timesplit_sex$MATURITY_F <- as.factor(NBI_timesplit_sex$MATURITY)

## for another type of comparison. The stages overlap at the start
NBI_timesplit_sex$TimeStart2 <- 0
## TimeStop2 is the difference of tstop - tstart. With this we take into account that the start time is 0 for all lines
NBI_timesplit_sex$TimeStop2 <- NBI_timesplit_sex$tstop - NBI_timesplit_sex$tstart
```
...
```{r stage surv curve life}
# life status
sfit_matu <- survfit(formula = Surv(time = NBI_timesplit_sex$TimeStart2,
                                      time2 = NBI_timesplit_sex$TimeStop2, 
...
```{r col sub}
NBI_3col <- subset(NBI, NBI$breeding_area == "Burghausen" | NBI$breeding_area == "Kuchl")
...
# Correct a typo in the original data
```
