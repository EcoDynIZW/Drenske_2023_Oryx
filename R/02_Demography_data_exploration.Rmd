...
Plot the whole Population size and the number of individuals per sex, per raising type and per colony per year.
```{r}
...
# Customized barplots
## Overall population
Show the values for each category
Build a matrix with them. The NAs between some values are there for some space between the categories 
...
```{r}
NBI_deliv <- NBI[NBI$mortality_cause == "delivered",]
NBI_proj <- NBI[NBI$mortality_cause != "delivered",]

# Here we consider the status delivered
